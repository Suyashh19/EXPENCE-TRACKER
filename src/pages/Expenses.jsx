          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/40">
                ...
                <th className="pb-6 pr-4 text-right text-xs font-black uppercase tracking-widest text-slate-400">Action</th>
              </tr>
            </thead>

            <tbody className="divide-y divide-white/20">
              {expenseList.map((exp) => (
                <tr key={exp.id} className="group hover:bg-white/20 transition-all">
                  <td className="py-6 pl-4 text-sm font-bold text-slate-500">
                  ...
                  <td className="py-6">
                    <span
                      className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase border ${getCategoryStyle(exp.category)}`}
                    >
                      {exp.category}
                    </span>
                  </td>
