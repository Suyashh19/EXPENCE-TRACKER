...
const Navbar = ({ monthlyBudget, allTotal, totalDays }) => {
  ...
  }, [monthlyBudget, allTotal, totalDays]);
  ...
  const velocityMeta = (v) => {
    if (v < 1) return { label: "Under budget pace", color: "text-emerald-600" };
    if (v < 1.2) return { label: "On budget pace", color: "text-yellow-600" };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs sm:text-sm font-semibold text-slate-400 flex flex-wrap items-center gap-2">
          Welcome back to Finora ✨
          {health && (
            <span
              className={`font-bold ${healthMeta[health.label]?.color}`}
            >
              {healthMeta[health.label]?.icon}{" "}
              {healthMeta[health.label]?.text}
            </span>
          )}
        </p>
      </div>
